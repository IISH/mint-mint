...
XSDMappingEditor.prototype.validate = function() {
	...
	$.ajax({
		...
		success: function(response) {
			tree.updateXpathMappings(response, self);
		}
	});
...
function targetHasMappings(target) {
	...
	var mappingCases = target['mapping-cases'];
	if (mappingCases != undefined && mappingCases[0] != undefined && mappingCases[0].mappings != undefined && mappingCases[0].mappings.length > 0) return true;
