...
XMLAnnotator.MODE_DEFAULT = XMLAnnotator.MODE_COMPLEX;
		...
		success: function(response) {
			if(response != undefined) {
				if(response.error != undefined) {
					alert("Error in tree retrieval" + response.error);
				} else if(response) {
					tree = response.tree;
...
XMLAnnotator.prototype.initAnnotationsContainer = function() {
	...
	$.ajax({
		...
		success: function(response) {
			...
			this.dataUploadId = response.dsId;
			
			 var panelcount=$('div[id^=kp]:last');
		     var panelid=panelcount.attr('id');
		     var pnum=parseInt(panelid.substring(2,panelid.length))-1;
		     var preprevPanel=$('#kp'+pnum);
		     $K.kaiten('reload', preprevPanel, {kConnector:'html.page', url:'DatasetOptions.action?uploadId='+this.dataUploadId+'&organizationId='+ooid+'&userId='+urid, kTitle:'Dataset Options'}, true);
...
XMLAnnotator.prototype.loadItem = function(item) {
	if(item == undefined) {
		if(this.loadedItem == undefined) return;
...
XMLAnnotator.prototype.showAnnotatorMode = function(views, template) {
	...
	$('#function-popup').dialog('destroy').remove();
...
XMLAnnotator.prototype.showGroupAnnotator = function(views, template) {
	...
	this.mode = XMLAnnotator.MODE_COMPLEX;
	//this.showGroupComplex();
...
XMLAnnotator.prototype.groupAnnotatorButtons = function(views) {
	...
	if (views != undefined && views.views[0].groupAnn != undefined) {
		var btnMode = $("<button>")
		.addClass("editor-toolbar-button")
		...
		.click(function () {
			if(self.mode == XMLAnnotator.MODE_SIMPLE) {
				self.mode = XMLAnnotator.MODE_COMPLEX;
				$(this).find(".ui-button-text").text("Switch to simple");
				self.container.annotations.empty();
				self.showGroupComplex();
			} else {
				self.mode = XMLAnnotator.MODE_SIMPLE;
				$(this).find(".ui-button-text").text("Switch to complex");		
				self.container.annotations.empty();
				self.showGroupSimple();
			}	
		}).appendTo(buttons);
		if (self.mode == XMLAnnotator.MODE_COMPLEX) {
			self.container.annotations.empty();
			self.showGroupComplex();
		}
	}
...
XMLAnnotator.prototype.showGroupComplex = function() {
	this.container.annotations.empty();
	$('#function-popup').dialog('destroy').remove();
	$("<div>").annotatorElement({
		target: tree,
		editor: this,
		schemaMapping: false
...
XMLAnnotator.prototype.showGroupSimple = function() {
...
XMLAnnotator.prototype.showMappingComplex = function(mapping) {
	//mapping may have been changed, have to refresh
...
XMLAnnotator.prototype.showMappingSimple = function() {
		...
		for(var i=0;i<groupActs.length;i++){
			...
			if(groupActs[i].type=="search"){
				...
				if(groupActs[i].arguments.queryType=="all"){
					...
					buildhtml+=	" in labels";}
				else if (groupActs[i].arguments.queryType=="field") {
					buildhtml+=" in xpath " + groupActs[i].arguments.xpath;
				}
				lastIsSearch = true;
			}
				...
				if (groupActs[i].arguments.length>0)
					if (groupActs[i].arguments[0] == -1)
						val=groupActs[i].arguments[1].length;
					else 
			...
			else if(groupActs[i].type == "appendToStringAfter"){
				buildhtml+="xpath: "+groupActs[i].xpath+" <br/> add: "+groupActs[i].arguments[1] +
				" after " + groupActs[i].arguments[0];
			}
			else if(groupActs[i].type == "appendToStringBefore"){
				buildhtml+="xpath: "+groupActs[i].xpath+" <br/> add: "+groupActs[i].arguments[1] +
				" before " + groupActs[i].arguments[0];
			}
			else if(groupActs[i].type == "appendAfter"){
				buildhtml+="xpath: "+groupActs[i].xpath+" <br/> add: "+groupActs[i].arguments[0] +
				" after current value";
			}
			else if(groupActs[i].type == "appendBefore"){
				buildhtml+="xpath: "+groupActs[i].xpath+" <br/> add: "+groupActs[i].arguments[0] +
			...
			else if (groupActs[i].type == "add new") {
				buildhtml+="xpath: "+groupActs[i].xpath;
			}
...
XMLAnnotator.prototype.initItemBrowser = function() {
	self = this;
	this.container.items.empty();
	this.items = $("<div>").appendTo(this.container.items).itemBrowser({
		datasetId: this.dataUploadId,
