(function($) {
	var widget = "annotatorElementSimpleView";
	var defaults = {
		'xpath': undefined,
	...
	$.fn.annotatorElementSimpleView = function(method) {
	    if ( methods[method] ) {
		...
		if (xpath != undefined && label != undefined) {
			...
			if (showDuplicateWithNewValue) {
				...
				editValue.click(function() {
					...
					if (value != null)
					   container.annotatorElementSimpleView("addCallData", "duplicate", xpath, [value]);
				})
			}
			if (showRemoveWithValue) {
				...
				editValue.click(function() {
					...
					if (value != null)
					   container.annotatorElementSimpleView("addCallData", "remove", xpath, [value]);
			...
			if (showRemoveAll) {
				...
				editValue.click(function() {
					...
					rpopup.dialog({
				        ...
				        "Confirm" : function() {
						  $(this).dialog("close");
						  container.annotatorElementSimpleView("addCallData", "remove", xpath, ["*"]);
				        },
				        "Cancel" : function() {
				          $(this).dialog("close");
				        }
