...
public class OrgOAIBeanFactory {
	...
	public OrgOAIBeanFactory() {
		...
		projectstatus = getProjectOAIBean();
		organizationitemnumberpernamespace = getOrganizationOaiItemsPerNamespace();
	...
	public List<OrgOAIBean> getOrgOAIBeans2(String OrganizationId){
		...
		while (it.hasNext()){
			...
			if (itemCountmap.containsKey(OrganizationId)){
				...
				String organizationName = DB.getOrganizationDAO().getById(Long.parseLong(OrganizationId), false).getEnglishName();	
				OrgOAIBean orgoaiBean = new OrgOAIBean(organizationName, namespace, OrganizationId, unique);
