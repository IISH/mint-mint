...
public class OrganizationGoalsSummaryBeanFactory {
	...
	public List<OrganizationGoalsSummaryBean> getOrgGoalBeans(String orgid,Integer transformed,Integer published,Integer oaipublished) {
		...
		Organization org = DB.getOrganizationDAO().getById(Long.parseLong(orgid), false);
		...
		if (org.getTargets() !=null){
			Tuple t  = org.getFinalTarget();
			finaltargetItems = (Integer) t.v;
			finalDate = (Date) t.u;

			t  = org.getCurrentTarget();
			targetItems = (Integer) t.v;
			targetDate = (Date) t.u;
		}
		Date today = endDate;
