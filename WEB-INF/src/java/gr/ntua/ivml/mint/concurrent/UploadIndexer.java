...
public class UploadIndexer implements Runnable, ReportI {
	...
	public boolean marcPreprocess = false;
	...
	private void convertJson() throws Exception {
		...
		try {
			...
			du.logEvent( "Error during json processing", e.getMessage() );
			du.setLoadingStatus( DataUpload.LOADING_FAILED);
			throw e;
		} finally {
			try {
	...
	/**
	 * Creates a new tmpfile archive with pretransformed XML via XSL.
	 */
	private void convertMarc() throws Exception {
		FileOutputStream fOut = null;
	    BufferedOutputStream bOut = null;
	    GzipCompressorOutputStream gzOut = null;
	    TarArchiveOutputStream tOut = null;
	    ...
	    final XSLTransform marcTrans = new XSLTransform();
		final String xsl = FileUtils.readFileToString(new File(Config.getProjectRoot(), "xsl/marc-importer.xsl"), "ISO8859_1" );

		try {
			newTmpFile = File.createTempFile("ConvertMarc", ".tgz");
			...
			EntryProcessorI ep = new EntryProcessorI() {
				...
				public void processEntry(String pathname, InputStream is)
					...
					TarArchiveEntry tarEntry = new TarArchiveEntry( pathname  );
					ByteArrayOutputStream bos = new ByteArrayOutputStream();
					
					marcTrans.transform( is, xsl, bos );
			        bos.flush();
					tarEntry.setSize(bos.size());
					tarOut.setLongFileMode(TarArchiveOutputStream.LONGFILE_GNU);
					tarOut.putArchiveEntry(tarEntry);
			...
			du.logEvent( "Started marc preprocessing.");
			FileFormatHelper.processAllEntries(tmpFile, ep);
			if( count.get() == 0 ) {
				throw new Exception( "No .xml file for conversion found!");
			}
			// use the converted one
			tmpFile.delete();
			tmpFile = newTmpFile;
			du.logEvent( "Finished marc preprocessing.", "Processed " + count.get() + " files.");
		} catch( Exception e ) {
			// just for some cleanup
			if( tmpFile != null ) {
				tmpFile.delete();
			...
			du.logEvent( "Error during marc preprocessing", e.getMessage() );
			du.setLoadingStatus( DataUpload.LOADING_FAILED);
