...
public class XSLUtils {
	...
	public static String xslValueOf(String select) {
		select = StringEscapeUtils.escapeXml(select);
		if(select == null) {
			return "<xsl:value-of select=\".\"/>";
		} else {
			if( select.contains("\"")) select = select.replace("\"", "&quot;");
	...
	public static String function(Function function) {
		...
		if(function != null && function.hasCall() && function.hasArguments()) {
			...
			for(int a = 0; a < args.size(); a++) {
				if (call.equalsIgnoreCase(Function.FUNCTION_CALL_TOKENIZE) || 
						call.equalsIgnoreCase(Function.FUNCTION_CALL_SPLIT))
					arguments.add(XSLUtils.escapeConstant(args.get(a).toString()));
				else
					arguments.add(args.get(a).toString());	
			}
			...
			} else if (call.equalsIgnoreCase(Function.FUNCTION_CALL_REPLACE_REGEX)) {
				result = XSLUtils.xslValueOf("replace(., '" + arguments.get(0).toString() + "', '" + arguments.get(1).toString() + "')");
			} else if (call.equalsIgnoreCase(Function.FUNCTION_CALL_REPLACE_STRING)) {
				result = XSLUtils.xslValueOf("replace(., '" + escapeRegexp( arguments.get(0).toString()) + "', '" + arguments.get(1).toString() + "')");
		...
		}else if(function != null && function.hasCall()  && function.getCall().equalsIgnoreCase(Function.FUNCTION_CALL_TRIM)){
