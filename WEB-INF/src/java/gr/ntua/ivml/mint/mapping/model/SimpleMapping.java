...
public class SimpleMapping extends JSONHandler {
	...
	public void trimValues() {
		if(this.has(MAPPING_VALUE_MAPPINGS)) 
			this.getValueMappings().trimValues();
	}
	
	public void sortValues() {
		if(this.has(MAPPING_VALUE_MAPPINGS)) 
			this.getValueMappings().sortValues();
	}
	
	public void removeDuplicates() {
		if(this.has(MAPPING_VALUE_MAPPINGS)) 
			this.getValueMappings().removeDuplicates();
	}
	
	public boolean hasDuplicates() {
		if(this.has(MAPPING_VALUE_MAPPINGS)) 
			return this.getValueMappings().hasDuplicates(new HashSet<String>(), 
					this.getValueMappings().asJSONArray().iterator());
		return false;
	}
	...
	public ValueMappings getValueMappings() {
		...
		ValueMappings valueMaps = new ValueMappings(this.getArray(MAPPING_VALUE_MAPPINGS));
		return valueMaps;
	}
	...
	public void setValueMapping(String input, String output, String thesLang) {
		this.getValueMappings().set(input, output, thesLang);
	}
