...
public class MappingCache {
	...
	public Element duplicate(String id) {
		...
		int indexC = parent.getByIndex(Element.ELEMENT_CHILDREN, id, original);
		int indexA = parent.getByIndex(Element.ELEMENT_ATTRIBUTES, id, original);
		if(indexC >= 0 || indexA >=0) {
			Element duplicate = original.duplicate();
			if (indexC >= 0) {
				duplicate.setRemovable(true);
				parent.addChild(indexC, duplicate);
			}
			else if (indexA >= 0)
				parent.addAtIndex(indexA, Element.ELEMENT_ATTRIBUTES, duplicate);
	...
	/*
	 * Method for duplicating the parent of an attribute
	 */
	public Element duplicateAttribute(String id) {
		Element parent = this.getParentHandler(id);
		Element grandparent = this.getParentHandler(parent.getId());
		String parentId = parent.getId();
		int index = grandparent.getByIndex(Element.ELEMENT_CHILDREN, parentId);
		
		if(index >= 0) {
			Element duplicate = parent.duplicate();
			duplicate.setRemovable(true);
			grandparent.addChild(index, duplicate);
			this.fillEmptyIdsRecursive(duplicate, true);
			this.cacheElement(duplicate, grandparent);
			return duplicate;
		}
