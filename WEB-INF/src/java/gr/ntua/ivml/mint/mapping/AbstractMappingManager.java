...
public abstract class AbstractMappingManager {	
	...
	@Deprecated
	public JSONObject setValueMapping(String input, String output, String target, int index, String thesLang) {
		return this.setValueMapping(new MappingIndex(target, index, -1), input, output, thesLang);
	}
	
	public JSONObject setValueMapping(MappingIndex index, String input, String output, String thesLang) {
		Element element = this.cache.getElementHandler(index.getId());
		setValueMapping(index, input, output, element, thesLang);
		save();
		
		return element.asJSONObject();		
	}
	
	public void setValueMapping(MappingIndex index, String input, String output, Element element, String thesLang) {
		SimpleMapping mapping = element.getMapping(index);
		if(mapping != null) mapping.setValueMapping(input, output, thesLang);
	}
